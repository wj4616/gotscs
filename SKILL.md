...
Run via Bash tool:
```bash
~/.claude/skills/gotscs/scripts/validate-graph.sh --expect-nodes 19 --expect-edges 52
```
Expected: `PRC1 PASS: 19 nodes (2 conditional), 52 edges`
...
The Edge Table contains 52 rows. Edge type breakdown: required=14, optional=2, gate-open=3, forward-conditional=19, back-edge=9, terminal=5. Closed-vocab enforcement via `graph.schema.json` (HC-03).
