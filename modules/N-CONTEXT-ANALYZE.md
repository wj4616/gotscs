...
2. **Branch on input class.**
   ...
   **For ec-skill (`--context <skill-dir>`):**
   ...
   c. Extract every node from the existing graph.json. For each node, classify as:
   ...
   d. Emit `context_inventory` markdown with the classification table (rows: `node_id | classification | rationale | structural_defect (if replace-target) | context_source | context_rationale`) plus a `pipeline_class` line.
