...
2. **Assemble the rendered SKILL.md document.** The output MUST contain ALL of the following sections in this exact order:
   ...
   **§0 EVOLUTION_MODE DECLARATION (NEW v4.3 Phase 3 — only when GOTSCS itself ran in evolve+ mode).** Read `evolution_mode` from `stages/N-PREFLIGHT.md`. If `evolution_mode in {evolve, evolve-aggressive}`: render the following block immediately after §0 INVOCATION. **Heading style:** `## GENESIS` (no `§0` prefix in the rendered heading — matches sibling §0 sub-sections HARD GATES and ARCHITECTURE which use bare `## ` headings; the `§0` is only used in this template's describing prose, not in the produced SKILL.md text):

   ```markdown
   ## GENESIS

   This skill was generated by GOTSCS in `evolution_mode: <evolve | evolve-aggressive>`. Its design was synthesized from multiple context sources via the precedence stack P1 brief > P2 spec > P3 original > P4 default. See `FUSION.md` for the full audit trail of every divergence from the original skill, and `REGRESSION.md` for the regression test plan. See `RATIONALE.md` for the full design rationale including fusion redesign justifications.

   waiver_justification: <verbatim string from stages/waiver_justification.txt; only present when evolution_mode == 'evolve-aggressive'>
   fusion_sources_count: <integer count from N-FUSION-ANALYZE.context_inventory_classified>
   fusion_decisions_count: <integer count from N-FUSION-ANALYZE.fusion_decisions>
   ```

   When `evolution_mode in {overlay, greenfield}`: the §0 GENESIS block is NOT emitted (preserves byte-equivalence with v4.2 produced skills under those modes).
   ...
   **§5 Optimizations:** From `stages/N-SYNTH-GRAPH.md § optimizations` AND `stages/N-FUSION-ANALYZE.md § fusion_decisions` (when present in evolve mode).

   **M6 fix — produced-skill scope enforcement (no GOTSCS-internal leakage).** The §5 Optimizations section in the *produced* SKILL.md MUST be derived solely from skill-specific synthesis decisions. Do NOT copy GOTSCS-internal text such as:
   - "PRC1 expect-edges 58→59" or other GOTSCS edge-count history
   - "v4.2 deltas" / "v4.3 deltas" sub-headings (those describe GOTSCS itself, not the produced skill)
   - "Briefing split (DD-03)" / "exec-type promotions (DD-04)" / "V-check left-shifts (DD-06)" — DD-NN markers refer to GOTSCS design decisions, not the produced skill's
   - GOTSCS Goal-1..Goal-8 text
   - "Token-budget cuts vs v3.1.0" or any compression-vs-prior-GOTSCS-version commentary

   Before emit, scan the assembled §5 content with the regex `(?i)(GOTSCS|expect-edges \d+|DD-0[0-9]|Goal-[0-9]|v3\.1\.0|v4\.[123]\.0)`. If any match is found in §5: HALT with `halt-on-skill-render-gotscs-leakage` listing the matched line numbers. Allowed: skill-specific FD-NN (fusion decisions) and skill-specific named anti-patterns (AP-NEW-1, AP-NEW-2, AP-NEW-3, etc.) when they originate in the produced skill's brief, not GOTSCS itself.
   ...
   **§8 RUNTIME CONVENTIONS (NEW v4.2 — DD-11).** Inline a self-contained reference block so the produced skill is **portable without GOTSCS installed**. Build content in three subsections by scanning the skill artifacts for actual references:

   1. **Halt-condition strings** — grep all module files (`stages/modules/*.md`) and the rendered SKILL.md body for tokens matching `halt-on-[a-z][a-z0-9-]+`. Emit a deduplicated table: `| halt-condition | meaning |`. Example: `halt-on-partial-coverage` → "fired when a validator-class node leaves any input claim without a verdict."

   2. **Signal-field names** — grep `stages/N-EDGES.md` and the modules for every `signal_field:` value. Emit a deduplicated table: `| signal_field | producer node | semantics |`. This lists the names of every cross-edge data signal the skill propagates (e.g., `aggregation_result`, `web_degraded`, `verification_failure`).

   3. **Named anti-patterns referenced in skill prose** — grep the rendered SKILL.md for `AP-[VTS][0-9]+(-LOCAL)?` patterns. For each match, inline a one-line definition (sourced from `stages/N-CONSTRAINTS.md` § anti-patterns, which itself preserves any `-LOCAL` overrides chosen at audit time). If a referenced AP has no inline definition available, emit a `render_warnings: undefined-AP-reference` note to the stage file (don't halt; surface to N-VERIFY).

   The full §8 block is typically ~30–50 lines depending on the skill's surface area. It REPLACES the old "Appendix A — see briefing-core.md" stub. The produced skill no longer references any external GOTSCS file at runtime.

   **DELETED v4.2 (DD-11):** The prior `## Appendix A — H.1-H.9 Schema Reference` stub that pointed to briefing-core.md is removed. H.1–H.9 are GOTSCS build-time heuristics (decomposition, topology selection, etc.) and are not relevant to the *runtime* of produced skills. Their reference in produced SKILL.md was vestigial.
...
3. **Self-audit before emission (V11 pre-check).** Before writing, verify:
   ...
   - **§8 RUNTIME CONVENTIONS is present** (NEW v4.2). At minimum the three sub-headings (halt-conditions, signal-fields, named anti-patterns) must be emitted. Empty tables under any heading are allowed for skills with no references in that category, but the headings themselves must be present.
