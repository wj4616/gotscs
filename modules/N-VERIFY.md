...
2.3. **V22 (token-budget-arithmetic — G-09).** Read `stages/N-WAVES.md § per_mode_token_budget_totals`. For each mode row: re-sum `token_budget` from N-REGISTRY active-wave nodes; compare to the row's `total_tokens`. If any mode's computed total differs from the stage-file value by >10%: add `'V22'` to repair_targets with advisory `"token-budget-arithmetic-drift: mode=<X> claimed=<Y> computed=<Z>"`. Non-blocking (advisory; does not affect verify_pass). If `stages/N-WAVES.md` lacks the `## per_mode_token_budget_totals` section: emit advisory `"V22-missing: N-WAVES did not emit per-mode totals table"`.
